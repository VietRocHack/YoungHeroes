"""Real-time voice-call handling via Gemini's Live API — see
...
# Matches PracticeCall.jsx's mic capture — see frontend/src/lib/audio.js.
...
_END_CALL_TOOL = types.Tool(
    function_declarations=[
        types.FunctionDeclaration(
            name="end_call",
            description=(
                "Call this exactly once, when the practice 911 call should end — either "
...
- This is a real-time conversation, not a turn-by-turn exchange — if the child
...
async def _relay_from_client(websocket: WebSocket, session, stop_event: asyncio.Event) -> None:
    ...
    text frame (rather than binary audio) is treated as a manual hangup —
    PracticeCall.jsx's "End Call" button — same as today's handleEndCall."""
    try:
            ...
            if data is not None:
                await session.send(
                    input=types.LiveClientRealtimeInput(
                        media_chunks=[types.Blob(data=data, mime_type=_INPUT_MIME_TYPE)]
                    )
                )
...
async def _relay_from_model(websocket: WebSocket, session, call_id: str, stop_event: asyncio.Event) -> bool:
    """Forward the model's spoken audio to the browser, and watch for the
    end_call tool call. Returns the final is_prank_call value."""
    is_prank_call = False
        ...
        if message.tool_call:
            for call in message.tool_call.function_calls:
                if call.name == "end_call":
                    is_prank_call = bool((call.args or {}).get("is_prank_call", False))
                    await session.send(
